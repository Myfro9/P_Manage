...
####### 导入缺料表
FileNameStr = 'PurchaseTable2021-06-28-15_52.xls'
FolderNameStr = './Results/PurchaseBOM_2021-06-28-15_52/'

####### 输入需要处理的生产计划单号
Target = 'RW202106-H'

time_info = FileNameStr.strip('PurchaseTable')
time_info = time_info.strip('.xls')
FolderNameStr1 = './Results/' + Target + '_询价表_' + time_info +'/'
pd_BOMList_need2Buy = pd.DataFrame(pd.read_excel(FolderNameStr+FileNameStr))
pd_BOMList_need2Buy = pd_BOMList_need2Buy[pd_BOMList_need2Buy['生产计划单号'].str.contains(Target).fillna(False)]
FileNameStr_Target = Target + '_' + time_info
...
if not os.path.exists(FolderNameStr1):
